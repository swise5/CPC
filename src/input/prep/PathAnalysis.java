...
//				double [] coords = cv.wgs84toOSGB36(Double.parseDouble(bits[7]), Double.parseDouble(bits[8]));
	...
	public void mapStatusesToRoadnames(String dirname, String outdir, String outsuffix, String fileType){
		try{
		File folder = new File(dirname);
		File[] runFiles = folder.listFiles();
	    SimpleDateFormat ft = new SimpleDateFormat ("dd/MM/yyyy HH:mm:ss");
		...
		// Create the output file
		BufferedWriter w = new BufferedWriter(new FileWriter("goldRecordsSnappedToEdges.txt"));//outdir + filename.split(fileType)[0] + outsuffix + ".txt"));
...
//				if(bits[13].equals(lastStatus) && timeDiff < 10) 
//					continue;
...
//				double [] coords = cv.wgs84toOSGB36(Double.parseDouble(bits[7]), Double.parseDouble(bits[8]));
				...
				int timeOutput = (int)((myDate.getTime() - startDate.getTime()) / 60000.);
				if(closestEdge != null)
					myResults += timeOutput + "\t" + bits[13] + "\t" + bits[0] + "\t" + bits[1] +"\t" + c.toString() + "\t" + ((MasonGeometry)closestEdge.info).getStringAttribute("FID_1") +"\n";
...
//				System.out.println(timeDiff / 60000.);
				...
				ListEdge closestEdge = getClosestEdge(c, EmergentCrime.spatialResolution);
				if(closestEdge != null)
					updateEdgeHeatmap(closestEdge);
...
//				w.write(s + "\n");
	...
	public PathAnalysis() {
		try {
			...
			roads = NetworkUtilities.multipartNetworkCleanup(roadLayer, roadNodes, EmergentCrime.spatialResolution, fa, random, 0); // TODO set random
			...
			for(Object o: roadNodes){
				...
				for(Object ed: roads.getEdgesOut(n)){
					...
					roadnames.add(myInfo.getStringAttribute("FID_1"));
				}
			}
			
			int myHelper = 0;
			for(String hello: roadnames){
...
//			emergencyPathsFromCAD(dataDir, dataDir + "cleaned/", "boops", fileType);
			...
			BufferedWriter myKmls = new BufferedWriter(new FileWriter("/Users/swise/testGOLDEN.kml"));
...
/*				String edgey = "<Placemark><name>" + myName //filename 
				...
				edgey += "\t</coordinates>\n</LineString>\n</Placemark>\n";
	*/			String edgey = value + "\t" + ((MasonGeometry)e.getInfo()).getStringAttribute("FID_1") + "\n";
			myKmls.write(edgey);
			}
